...
const docSync_1 = require("./sync/docSync");
...
class ChatPanel {
    static createOrShow(context) {
        ...
        const panel = vscode.window.createWebviewPanel(ChatPanel.viewType, "Odysseus", { viewColumn: vscode.ViewColumn.Beside, preserveFocus: true }, {
            enableScripts: true,
    ...
    constructor(panel, context) {
        ...
        this.disposables = [];
        this.panel = panel;
        this.panel.iconPath = vscode.Uri.parse("data:image/svg+xml," +
            encodeURIComponent("<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'><path d='M16 4L16 22L6 22Z' fill='#e06c75'/><path d='M16 8L16 22L24 22Z' fill='#e06c75' opacity='0.6'/><path d='M4 24Q10 20 16 24Q22 28 28 24' stroke='#e06c75' stroke-width='2.5' fill='none' stroke-linecap='round'/></svg>"));
    ...
    async init() {
        ...
        this.docSync = new docSync_1.DocSync(this.client);
        try {
    ...
    async ensureSession() {
        const savedId = this.context.workspaceState.get("odysseus.sessionId");
        ...
        const session = await this.client.createSession(`${folderName} (VS Code)`, this.currentModel || undefined, this.currentEndpointUrl || undefined);
        this.sessionId = session.id;
        if (session.model) {
            this.currentModel = session.model;
        }
        if (session.endpoint_url) {
            this.currentEndpointUrl = session.endpoint_url;
        }
    ...
    async handleMessage(msg) {
        switch (msg.type) {
            case "_jsError":
                console.error("[Odysseus] webview JS error:", msg.text);
                break;
            case "send":
                await this.handleSend(String(msg.text ?? ""), msg.opts);
                break;
            case "requestModels":
                await this.sendModelsToWebview();
                break;
            case "selectModel":
                await this.handleSelectModel(String(msg.model ?? ""), String(msg.endpointUrl ?? ""));
                break;
            case "newSession":
    ...
    async handleSelectModel(model, endpointUrl) {
        if (!model || !this.client || !this.sessionId) {
            return;
        }
    ...
    async handleSend(text, opts = {}) {
        if (!text.trim() || !this.client || !this.sessionId || !this.docSync) {
            return;
        }
        const token = await this.context.secrets.get("odysseus.token");
        const url = this.getUrl();
        const cfg = vscode.workspace.getConfiguration("odysseus");
        const agentMode = opts.agentMode ?? cfg.get("agentMode", true);
        const allowBash = opts.allowBash ?? cfg.get("allowBash", true);
        const allowWebSearch = opts.allowWebSearch ?? cfg.get("allowWebSearch", true);
        const fileCtx = (0, fileContext_1.getActiveFileContext)();
        const selCtx = (0, fileContext_1.getSelectionContext)();
        const workspaceRoot = (0, fileContext_1.getWorkspaceRoot)();
        const displayMessage = (0, fileContext_1.buildDisplayMessage)(text, selCtx);
        const apiMessage = (0, fileContext_1.buildApiMessage)(displayMessage, workspaceRoot, fileCtx);
        this.postMessage({ type: "userMessage", text: displayMessage });
        this.postMessage({ type: "assistantStart" });
        const writtenPaths = [];
        try {
            await (0, streaming_1.streamChat)({
                baseUrl: url,
                token,
        ...
        for (const p of writtenPaths) {
            try {
                ...
                const editor = vscode.window.visibleTextEditors.find((e) => e.document.uri.fsPath === p);
                if (editor) {
